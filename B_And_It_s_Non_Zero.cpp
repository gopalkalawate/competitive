...
vector <vector<int>> num0(200001,vector <int> (32,0));
...
void soln(){
    ...
    int ans = m-n+1;
    for(int i=0;i<32;i++){
        ans = min(ans,num0[m][i]-num0[n-1][i]);
    }
    cout<<ans<<'\n';
}
...
for(int i=1;i<200001;i++){
    for(int j=0;j<32;j++){
        num0[i][j] = num0[i-1][j] + (!((i>>j)&1));  // i>>j shifts right the bits of i by j 
                                                  // and brings the jth bit to the least significant place
    }
}
