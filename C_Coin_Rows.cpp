...
void soln(){
    ...
    vector <int> presumup(n+1,0),presumdown(n+1,0);

    for(int i=1;i<=n;i++){
        presumup[i] += a[i-1] + presumup[i-1];
        presumdown[i] += b[i-1] + presumdown[i-1];
    }

    //cout<<presumup<<presumdown<<'\n';

    int ans = INT_MAX;
    for(int i=1;i<=n;i++){
        ans = min(ans,max(presumup[n]-presumup[i],presumdown[i-1]));
    }
